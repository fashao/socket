...
public class AlarmKlaxon extends Service {
    ...
    public  final Handler mHandler_toast = new Handler(){
    	
    	public void handleMessage(Message msg) {
    		switch (msg.what) {
			case showToastOK:
				ToastMaster.showtoast(AlarmKlaxon.this,AlarmKlaxon.this.getString(R.string.sendOrderok));
				break;
			case showToastFail:
				ToastMaster.showtoast(AlarmKlaxon.this,AlarmKlaxon.this.getString(R.string.network_failed));
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
		...
		String wokingtable = PreferencesUtils.getString("wokingtable", "alarms");
		String oldtable = intent.getStringExtra("wokingtable");
		System.out.println("wokingtable: "+wokingtable +" oldtable ："+oldtable);
        if (oldtable!=null&&!oldtable.equals(wokingtable)) {
            Log.v("wangxianming", "time mode is not the same==");
            return START_NOT_STICKY;
		}
        ...
        System.out.println("AlarmKlaxon runnig....");
        controlSocket(AlarmKlaxon.this, alarm.socket, alarm.flag);
        play(alarm);
        mCurrentAlarm = alarm;
    ...
    private void play(Alarm alarm) {
        ...
        System.out.println("silent :"+alarm.silent +" alert "+ alarm.alert);
        
        if (!alarm.silent&&alarm.alert!=null) {
	...
	public   void controlSocket(final Context context, final int socket,final boolean ischeck) {
		System.out.println("controlSocket() socket" +socket+ " flag" +ischeck);
...
//						});
			...
			String result	 = socketUtils.clientget(SocketUtils.SETACTION, PreferencesUtils.getString(PreferencesUtils.PHONE,"null")+":"+(socket/6)+","+socket+":"+(ischeck?1:0));
